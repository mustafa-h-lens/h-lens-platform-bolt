...
export default function ClientAuth({ onSuccess }: ClientAuthProps) {
  ...
  if (step === 'otp') {
    // OTPInput accepts either phone or email and branches accordingly.
    return (
      <OTPInput
  ...
  return <ClientLoginPage onOTPSent={handleOTPSent} />;
}
    ...
    try {
      ...
      if (mode === 'phone') {
        ...
        const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-otp-sms`, {
          ...
          body: JSON.stringify({ phone: phoneE164, deviceInfo, portal_type: 'client' }),
        });
        ...
        const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-otp-email`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}` },
          body: JSON.stringify({ email: trimmedEmail, deviceInfo, portal_type: 'client' }),
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'حدث خطأ أثناء إرسال رمز التحقق');
  ...
  return (
    <div className="sl-page" data-dark={d ? '' : undefined}>
        ...
        <div className="sl-card-wrap">
          ...
          <div className="sl-card">
            ...
            <div className="sl-title-wrap">
              <h1 className="sl-title">بوابة العميل</h1>
      ...
      <style>{`
        ...
        .sl-blob { position: absolute; border-radius: 50%; filter: blur(80px); pointer-events: none; opacity: 0.7; }
        .sl-blob-1 { width: 500px; height: 500px; top: -10%; right: -8%; background: rgba(37,99,235,0.18); animation: blobFloat1 14s ease-in-out infinite; }
        .sl-blob-2 { width: 400px; height: 400px; bottom: -5%; left: -5%; background: rgba(124,58,237,0.15); animation: blobFloat2 16s ease-in-out infinite; }
        .sl-blob-3 { width: 350px; height: 350px; top: 40%; left: 10%; background: rgba(6,182,212,0.12); animation: blobFloat3 18s ease-in-out infinite; }
        .sl-blob-4 { width: 300px; height: 300px; top: 15%; right: 30%; background: rgba(16,185,129,0.10); animation: blobFloat4 20s ease-in-out infinite; }
        .sl-blob-5 { width: 250px; height: 250px; bottom: 20%; right: 15%; background: rgba(244,63,94,0.08); animation: blobFloat5 15s ease-in-out infinite; }
        ...
        @keyframes blobFloat1 { 0%, 100% { transform: translate(0,0) scale(1); } 33% { transform: translate(-30px,40px) scale(1.05); } 66% { transform: translate(20px,-20px) scale(0.95); } }
        @keyframes blobFloat2 { 0%, 100% { transform: translate(0,0) scale(1); } 33% { transform: translate(40px,-30px) scale(1.08); } 66% { transform: translate(-20px,25px) scale(0.96); } }
        @keyframes blobFloat3 { 0%, 100% { transform: translate(0,0) scale(1); } 50% { transform: translate(35px,30px) scale(1.04); } }
        @keyframes blobFloat4 { 0%, 100% { transform: translate(0,0) scale(1); } 50% { transform: translate(-25px,-35px) scale(1.06); } }
        @keyframes blobFloat5 { 0%, 100% { transform: translate(0,0) scale(1); } 33% { transform: translate(20px,-25px) scale(1.03); } 66% { transform: translate(-15px,20px) scale(0.97); } }
        .sl-grid { position: absolute; inset: 0; background-image: radial-gradient(rgba(0,0,0,0.04) 1px, transparent 1px); background-size: 28px 28px; pointer-events: none; z-index: 1; }
        .sl-page[data-dark] .sl-grid { background-image: radial-gradient(rgba(255,255,255,0.025) 1px, transparent 1px); }
        .sl-topbar { display: flex; align-items: center; justify-content: space-between; padding: 20px 28px; position: relative; z-index: 10; direction: ltr; }
        .sl-theme-btn { width: 46px; height: 46px; border-radius: 13px; background: var(--sl-theme-bg); border: 1px solid var(--sl-theme-border); backdrop-filter: blur(12px); display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; }
        .sl-theme-btn:hover { transform: scale(1.05); }
        .sl-logo { height: 54px; object-fit: contain; }
        .sl-main { flex: 1; display: flex; align-items: center; justify-content: center; padding: 0 1rem 3rem; position: relative; z-index: 2; }
        .sl-card-wrap { width: 100%; max-width: 440px; }
        .sl-card { background: var(--sl-card-bg); border: 1px solid var(--sl-card-border); border-radius: 24px; padding: 2.25rem 2rem 2rem; backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px); box-shadow: var(--sl-card-shadow); position: relative; overflow: hidden; }
        .sl-card-shine { position: absolute; top: 0; left: 0; right: 0; height: 1px; background: var(--sl-shine); }
        .sl-icon-wrap { display: flex; justify-content: center; margin-bottom: 22px; position: relative; width: 100px; height: 100px; margin-left: auto; margin-right: auto; }
        .sl-pulse { position: absolute; border-radius: 20px; }
        .sl-pulse-1 { inset: 0; background: var(--sl-pulse-1); animation: slPulse1 2.2s ease-out infinite; }
        .sl-pulse-2 { inset: -8px; border-radius: 24px; background: var(--sl-pulse-2); animation: slPulse2 2.2s ease-out 0.7s infinite; }
        @keyframes slPulse1 { 0% { transform: scale(1); opacity: 0.7; } 70% { transform: scale(1.5); opacity: 0; } 100% { transform: scale(1.5); opacity: 0; } }
        @keyframes slPulse2 { 0% { transform: scale(1); opacity: 0.45; } 70% { transform: scale(1.75); opacity: 0; } 100% { transform: scale(1.75); opacity: 0; } }
        .sl-icon-box { width: 68px; height: 68px; border-radius: 17px; background: var(--sl-icon-grad); border: 1px solid var(--sl-icon-border); display: flex; align-items: center; justify-content: center; box-shadow: var(--sl-icon-shadow); position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 2; color: var(--sl-icon-stroke); }
        .sl-title-wrap { text-align: center; margin-bottom: 22px; }
        .sl-title { font-size: 1.7rem; font-weight: 900; color: var(--sl-text); margin: 0 0 8px; }
        .sl-subtitle { font-size: 0.88rem; color: var(--sl-text-sub); line-height: 1.7; margin: 0; }
        /* Channel tabs (phone | email) — same pattern as vendor login */
        .sl-tabs {
          display: flex; gap: 6px; padding: 4px; margin-bottom: 18px;
          background: var(--sl-input-bg); border: 1px solid var(--sl-input-border);
          border-radius: 12px; backdrop-filter: blur(8px);
        }
        .sl-tab {
          flex: 1; display: inline-flex; align-items: center; justify-content: center;
          gap: 8px; padding: 10px 14px;
          border: none; background: transparent; color: var(--sl-text-muted);
        ...
        .sl-tab-active {
          background: #3b82f6; color: #fff; box-shadow: 0 4px 14px rgba(59,130,246,0.28);
        }
        .sl-page[data-dark] .sl-tab-active {
          background: rgba(59,130,246,0.85); box-shadow: 0 4px 14px rgba(59,130,246,0.35);
        }

        .sl-field-wrap { margin-bottom: 14px; }
        .sl-label { display: block; font-size: 0.82rem; font-weight: 700; color: var(--sl-text-sub); margin-bottom: 8px; text-align: right; }
        .sl-input { width: 100%; padding: 12px 14px; border-radius: 11px; background: var(--sl-input-bg); border: 1px solid var(--sl-input-border); color: var(--sl-text); font-family: inherit; font-size: 0.92rem; outline: none; transition: border 0.2s, box-shadow 0.2s; box-sizing: border-box; backdrop-filter: blur(8px); }
        .sl-input-focus { border-color: rgba(59,130,246,0.6) !important; box-shadow: 0 0 0 3px rgba(37,99,235,0.12); }
        .sl-input-error { border-color: rgba(239,68,68,0.5) !important; }
        ...
        /* Email input — mirrors phone input minus the prefix chip */
        .sl-email-wrap {
          display: flex; align-items: stretch;
          border-radius: 11px;
          background: var(--sl-input-bg); border: 1px solid var(--sl-input-border);
        ...
        .sl-email-input {
          ...
          background: transparent; border: none; color: var(--sl-text);
          font-family: ui-monospace, SFMono-Regular, monospace;
          font-size: 0.95rem; outline: none; width: 100%; text-align: left;
        }
        .sl-email-input::placeholder { color: var(--sl-text-muted); }
        .sl-error { margin-bottom: 12px; padding: 10px 13px; border-radius: 10px; background: var(--sl-error-bg); border: 1px solid var(--sl-error-border); backdrop-filter: blur(8px); }
        .sl-error p { font-size: 0.8rem; color: var(--sl-error-text); text-align: right; margin: 0; }
        .sl-submit { width: 100%; padding: 13px; border-radius: 11px; border: none; background: linear-gradient(135deg, #1d4ed8, #2563eb); color: white; font-family: inherit; font-size: 0.92rem; font-weight: 800; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 9px; box-shadow: 0 4px 20px rgba(37,99,235,0.35); transition: all 0.25s; margin-bottom: 16px; }
        .sl-submit:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(37,99,235,0.45); }
        .sl-submit:disabled { background: rgba(37,99,235,0.4); cursor: not-allowed; box-shadow: none; }
        .sl-info { padding: 11px 13px; border-radius: 10px; background: var(--sl-info-bg); border: 1px solid var(--sl-info-border); backdrop-filter: blur(8px); }
        .sl-info p { font-size: 0.75rem; color: var(--sl-text-muted); line-height: 1.7; text-align: right; margin: 0; }
