/*
  ...
  1. New Tables
    - `vendor_activity_log` - Tracks all vendor-related activities
    - `system_activity_log` - Tracks system-wide activities (POs, tasks, settings)

  2. Triggers
    - Vendor CRUD operations and status changes
    - Vendor equipment changes
    - Vendor document uploads/deletions
    - Vendor invoices/expenses CRUD and status changes
    - Purchase orders CRUD and status changes
    - Production tasks CRUD and status changes
    - Settings updates

  3. Views
    - Create global_activity_log view combining all activity sources

  4. Security
    - Enable RLS on all activity log tables
...
-- 2. Create system_activity_log table
...
CREATE INDEX IF NOT EXISTS idx_system_activity_log_entity_id ON public.system_activity_log(entity_id);
...
-- 3. Vendor triggers (CRUD + status changes)
